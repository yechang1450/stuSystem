...
//            String roleName = claims.get(CLAIM_KEY_AUTHORITIES).toString();
            ...
            List<SysResource> sysResources = sysResourceService.selectByUsername(username);
            userDetail = new UserDetail(userId, username, sysResources, "");
        } catch (Exception e) {
            userDetail = null;
        }
        return userDetail;
...
//                && !isCreatedBeforeLastPasswordReset(created, userDetail.getLastPasswordResetDate())
    ...
    public void putToken(String userName, String token) {
        redisUtil.hset(ROLE_REFRESH_TOKEN,userName,token);
        tokenMap.put(userName, token);
    }

    public void deleteToken(String userName) {
        redisUtil.hdel(ROLE_REFRESH_TOKEN,userName);
    ...
    public boolean containToken(String userName, String token) {
        ...
        if (userName != null) {
            if (!tokenMap.containsKey(userName)) {
                Object hget = redisUtil.hget(ROLE_REFRESH_TOKEN, userName);
                if (hget != null) {
                    tokenMap.put(userName, hget.toString());
                } else {
                    return false;
                }
            }
            if (tokenMap.get(userName).equals(token.trim())) {
                return true;
            }
        }
        return false;
